...
using namespace std;
namespace bpt {
// custom compare operator for STL algorithm 
...
inline typename T::child_t end(T &node){
    return begin(node) + node.n;
}

inline index_t *find(internal_node_t &node, const key_t &key){
    return upper_bound(begin(node), end(node) - 1, key);
}
inline record_t *find(leaf_node_t &leaf, const key_t &key){
    return lower_bound(begin(leaf), end(leaf), key);
}
...
bplus_tree::bplus_tree(const char *p, bool force_empty): fp(NULL), fp_level(0) {    
    ...
    if(force_empty){
        ...
        */
        open_file("w+");
        // create empty tree if file dosen't exist 
        init_from_empty();
...
int bplus_tree::insert(const key_t &key, value_t values){
    off_t parent_offset = get_parent_offset(key);
    off_t leaf_offset = search_leaf(parent_offset, key);
    leaf_node_t leaf;
    map(&leaf, leaf_offset);
    ...
    if(leaf.n == meta.order){
        ...
        node_create(leaf_offset, &leaf, &new_leaf);
        ...
        if(place_right){
            insert_record_into_leaf_node(&new_leaf, key, values);
        }else{
            insert_record_into_leaf_node(&leaf, key, values);
        }
        // save leafs
        unmap(&leaf, leaf_offset);
        unmap(&new_leaf, leaf.next);
        // insert new index key
        insert_key_to_index(parent_offset, new_leaf.children[0].key, leaf_offset, leaf.next);
    }else{
        insert_record_into_leaf_node(&leaf, key, values);
        unmap(&leaf, leaf_offset);
    }
...
off_t bplus_tree::get_parent_offset(const key_t &key) const{
    off_t currentNodeOffset = meta.root_offset;
    int height = meta.height;
    while(height > 1){
        internal_node_t node;
        map(&node, currentNodeOffset);

        index_t *index =  upper_bound(begin(node), end(node) - 1, key);
        currentNodeOffset = index->child;
        height--;
    }
...
void bplus_tree::insert_record_into_leaf_node(leaf_node_t *leaf, const key_t &key, const value_t &value){
...
void bplus_tree::insert_key_to_index(off_t offset, const key_t &key, off_t old, off_t after){
    ...
    if(node.n == meta.order){
        ...
        // update childern's parent
        reset_index_children_parent(begin(new_node), end(new_node), node.next);

        // give the middle key to the parent , node: middle key's child is reserved
...
void bplus_tree::insert_key_to_index_no_split(internal_node_t &node, const key_t &key, off_t child_offset){
...
off_t bplus_tree::search_leaf(const key_t &key) const{
    return search_leaf(get_parent_offset(key), key);
}
...
int bplus_tree::remove(const key_t &key){
    ...
    if(min_n > leaf.n){
        ...
        if(!borrowed){
            ...
            // remove parent's key
            remove_from_index(parent_offset, parent, index_key);
        }else{
            unmap(&leaf, leaf_offset);
        }
...
void bplus_tree::remove_from_index(off_t offset, internal_node_t &node, const key_t &key){
    ...
    if(to_delete != end(node)){
        ...
        copy(to_delete + 1, end(node), to_delete);
    }
    node.n--;
...
void bplus_tree::remove_leaf(leaf_node_t *prev, leaf_node_t *leaf){
    unalloc(true);
    prev->next = leaf->next;
    if(leaf->next != 0){
        leaf_node_t next;
...
bool bplus_tree::borrow_key(bool from_right, leaf_node_t &borrower){
    ...
    if(lender.n != meta.order / 2){
        record_t *whereToLend , *whereToPut;
       if (from_right) {
            whereToLend = begin(lender);
            whereToPut = end(borrower);
            adopt_key_in_parent_node(borrower.parent, begin(borrower)->key,
                                lender.children[1].key);
        } else {
            whereToLend = end(lender) - 1;
            whereToPut = begin(borrower);
            adopt_key_in_parent_node(lender.parent, begin(lender)->key,
                                whereToLend->key);
        }
        // store
        copy_backward(whereToPut, end(borrower), end(borrower) + 1);
        *whereToPut = *whereToLend;
        borrower.n++;

        // erase
        copy(whereToLend + 1, end(lender), whereToLend);
        lender.n--;
...
void bplus_tree::adopt_key_in_parent_node(off_t parent_offset, const key_t &prev, const key_t &new_key){
    internal_node_t node;
    map(&node, parent_offset);

    index_t *index = find(node, prev);
    index->key = new_key;
    unmap(&node, parent_offset); 
    if(index == node.children + node.n - 1){
        adopt_key_in_parent_node(node.parent, prev, new_key);
    }
}
template<class T>
